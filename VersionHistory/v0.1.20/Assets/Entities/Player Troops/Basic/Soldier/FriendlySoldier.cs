...
public class FriendlySoldier : MonoBehaviour {
            ...
            if ( conductor.measurePos < PlayerCommander.PLAYER_TURN_START_THRESHOLD || conductor.measurePos > PlayerCommander.PLAYER_TURN_END_THRESHOLD ) {
                ...
                if ( PlayerCommander.cmdIssued == Commands.MARCH_STR || PlayerCommander.cmdIssued == Commands.CHARGE_STR ) {
                    if ( lastBeat == 0 ) {
                        ...
                        if ( currMarkerPos < laneInterface.numMarkers ) {
                            marchDist = (laneInterface.distBtwMarkers * marchDistInMarkers) / 4f; //divide march into x amount of steps
                        } else {
                ...
                /*STEADY*/
                    ...
                    if ( lastBeat == 0 ) {
                        ...
                        if ( currMarkerPos < laneInterface.numMarkers ) {
                            marchDist = (laneInterface.distBtwMarkers * steadyDistInMarkers) / 4f; //divide march into x amount of steps
                        } else {
                            Destroy(gameObject); //destroy troop if at edge of the lane
                        }
                    }
    ...
    /*
    * Moves the entity to a given point to the right
    */
