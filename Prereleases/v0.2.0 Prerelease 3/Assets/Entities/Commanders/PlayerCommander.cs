...
public class PlayerCommander : MonoBehaviour {
    ...
    void Update() {
        ...
        } else if ( conductor.songStart && ( conductor.measurePos >= PLAYER_TURN_START_THRESHOLD && conductor.measurePos <= PLAYER_TURN_END_THRESHOLD ) ) {
            if ( !doValidateTurn ) {
                playerTurn = true;
                doValidateTurn = true;
            }
        ...
        if ( lastBeat != conductor.currBeat ) {
            ...
            if ( playerTurn ) {
                conductor.musicSources[1 - conductor.track].PlayOneShot(conductor.playerCue, 0.45f);
            }
            StartCoroutine( Bop() );
        }
