    ...
    */
    private IEnumerator MarchTo( int nextMarkerID, float nextMarkerDist ) {
        ...
        if ( nextMarkerID < laneInterface.numMarkers ) {
            GameObject nextMarker = currLane.transform.GetChild(nextMarkerID).gameObject;
        ...
        while ( true ) {
            if ( beatTick != conductor.currBeat ) {
                ...
                if ( cueAttack ) {
                    ...
                    EnemyTroop enemy = target.GetComponent<EnemyTroop>();
                    StartCoroutine( enemy.TakeDamage(atkDmg) );
